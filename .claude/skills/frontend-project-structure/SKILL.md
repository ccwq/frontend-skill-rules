...
- v1.5.0：将业务目录命名从 `src/features/`、`src/features-shared/` 调整为 `src/business/`、`src/business-shared/`；重排文件归属决策树，优先判断 API / Mock / store / router / 基础设施；放宽模块导出入口 `index.ts` / `index.js`；新增文件路由项目例外规则；明确 Vue 优先 `composables/`、React 优先 `hooks/`。
